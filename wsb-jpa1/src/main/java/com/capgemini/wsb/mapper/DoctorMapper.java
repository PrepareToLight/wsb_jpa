...
public final class DoctorMapper {
    public static DoctorTO mapToTO(final DoctorEntity doctorEntity) {
        ...
        if (doctorEntity.getAddressEntity() != null) {
            doctorTO.setAddressTO(AddressMapper.mapToTO(doctorEntity.getAddressEntity()));
        }
        if (doctorEntity.getVisitEntityList() != null) {
            doctorTO.setVisitTOList(doctorEntity.getVisitEntityList().stream().map(VisitMapper::mapToTO).collect(Collectors.toList()));
        }
        return doctorTO;
    }
    ...
    public static DoctorEntity mapToEntity(final DoctorTO doctorTO) {
        ...
        if (doctorTO.getAddressTO() != null) {
            doctorEntity.setAddressEntity(AddressMapper.mapToEntity(doctorTO.getAddressTO()));
        }
        if (doctorTO.getVisitTOList() != null) {
            doctorEntity.setVisitEntityList(doctorTO.getVisitTOList().stream().map(VisitMapper::mapToEntity).collect(Collectors.toList()));
        }
        return doctorEntity;
    }
}
